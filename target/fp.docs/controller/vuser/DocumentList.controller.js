sap.ui.controller("controller.vuser.DocumentList", {
	 ...
	 * initialization.
	 * 
	 * @memberOf controller.vuser.DocumentList
	 */
	onInit: function() {
		this.actions = {
			upload: "upload",
	...
	loadRequiredData: function() {
		...
		this.getView().getModel('inbox').setData(Connection.requestData(Configuration.vendorUserAccountServiceUrl.documentInBox));
	...
	onItemClickEvent: function(oEvent) {
		...
		if (isPasscodeSecured) {
			if (this.oDocument.contentType === "doc/folder") {
				...
				this.initDownload(oEvent.getSource());
			}
		} else {
			if (this.oDocument.contentType === "doc/folder") {
	...
	onItemSelectedEvent: function(oEvent) {
		...
		var oContentType = oDocument.contentType;
		this.toggleButton(oContentType);
	},
	...
	handleUpload: function(oEvent) {
		...
		var data = {
			openCmisID: oSelectedItem.openCmisID
		};
	...
	toggleButton: function(oContentType) {
		this.getView().byId('btnDownload').setVisible(!Formatter.isFolder(oContentType));
	},
	loadLocationContent: function(oOpenCmisDocumentID, oOpenCmisDocumentParent, oContentType) {
		...
		if (Formatter.isFolder(oContentType)) {
	...
	onAuthenticationPress: function(oEvent) {
		...
		if (isValid) {
			var data = {
				documentShareID: this.oDocument.documentShareID,
				passCode: passCode
			};
			var url = Configuration.vendorUserAccountServiceUrl.authenticate;
			Connection.postData(url, data,
				function(response) {
	...
	sharedBy: function(oFirstName, oLastName) {
		if (!oFirstName || !oLastName) {
			return "";
		}
		return "Shared By:" + oFirstName + " " + oLastName;
